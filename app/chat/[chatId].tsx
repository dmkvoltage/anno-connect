...
export default function ChatDetail() {
  ...
  const onViewableItemsChanged = useRef(({ viewableItems }: any) => {
    if (!user?.uid || !chatId || viewableItems.length === 0) return;
    ...
    const fetchLastReadAt = async () => {
      try {
        const readStatusDoc = await getDoc(
          doc(db, "chats", chatId as string, "readStatus", user.uid)
        );
  ...
  useEffect(() => {
    ...
    const fetchChatInfo = async () => {
      ...
      if (cachedChat) {
        const otherParticipantId = cachedChat.participants.find(
          ...
          if (otherParticipantId) {
            ...
            if (participantData) {
              UserCache.setUser({
                ...
                lastSeen:
                  participantData.lastSeen?.toDate?.()?.toISOString() ||
                  undefined,
                  ...
                  ? {
                      ...
                      createdAt:
                        chatData.lastMessage.createdAt
                          ?.toDate?.()
                          ?.toISOString() || chatData.lastMessage.createdAt,
    ...
    const unsubscribe = onSnapshot(
      ...
      async (snapshot) => {
        ...
        const filteredMessages = messagesData.filter((message) => {
          const deletedBy = message.deletedBy || [];
          return (
            !deletedBy.includes(user.uid) && !message.id.startsWith("temp-")
          );
        });
  ...
  useEffect(() => {
    ...
    for (let i = messages.length - 1; i >= 0; i--) {
      ...
      if (msg.senderId !== user?.uid && messageDate > lastReadAt) {
        lastUnreadIndex = i;
        break;
      }
    }
    ...
    initialScrollIndex.current =
      lastUnreadIndex !== -1 ? lastUnreadIndex : messages.length - 1;
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const currentUnreadCount =
        chatData?.unreadCount?.[participant?.id || ""] || 0;
  ...
  const renderMessage = ({ item }: { item: ChatMessage }) => {
    ...
    const getStatusIcon = () => {
      ...
      if (isRead) {
        ...
        // Double dark grey checkmarks for delivered
        return (
          <View style={styles.tickContainer}>
            <Text style={[styles.tick, styles.tickDelivered]}>✓✓</Text>
          </View>
        );
        ...
        <View style={styles.messageWrapper}>
          ...
          >
            ...
            {repliedMessage && (
              ...
              >
                <View
                  style={[
                    styles.replyLine,
                    isOwnMessage ? styles.replyLineOwn : styles.replyLineOther,
                  ]}
                />
                <View style={styles.replyContent}>
                  <Text
                    style={[
                      styles.replySender,
                      isOwnMessage
                        ? styles.replySenderOwn
                        : styles.replySenderOther,
                    ]}
                    numberOfLines={1}
                    ellipsizeMode="tail"
                  >
                    {repliedMessage.senderId === user?.uid
                      ? "You"
            ...
            <View style={styles.messageFooter}>
              <Text
                style={
                  isOwnMessage ? styles.messageTimeOwn : styles.messageTime
                }
              >
                {timeAgo}
              </Text>
            ...
            >
              ...
              <View style={styles.actionsList}>
                <TouchableOpacity
                  ...
                  onPress={() => startReplying(selectedMessage)}
                >
                  <View
                    style={[styles.actionIconContainer, styles.replyIconBg]}
                  >
                    <ArrowLeft
                      size={20}
                      color="#013a96da"
                ...
                {canEdit && (
                  <>
                    ...
                    >
                      <View
                        style={[styles.actionIconContainer, styles.editIconBg]}
                      >
                        <Edit3 size={20} color="#013a96da" />
                      </View>
                ...
                {!isOwnMessage && (
                  ...
                  >
                    <View
                      style={[styles.actionIconContainer, styles.deleteIconBg]}
                    >
                      <Trash2 size={20} color="#f19809ff" />
                    </View>
                    <Text style={[styles.actionText, styles.deleteText]}>
                      Delete for Me
                    </Text>
      ...
      >
        ...
        <FlatList
          ...
          onScrollToIndexFailed={(info) => {
            const { index, highestMeasuredFrameIndex, averageItemLength } =
              info;
        ...
        {/* Input */}
        <View
          style={[styles.inputContainer, { marginBottom: insets.bottom || 8 }]}
        >
...
const updateLastReadAt = async (chatId: string, userId: string) => {
  ...
  try {
    await setDoc(
      doc(db, "chats", chatId as string, "readStatus", userId),
      { lastReadAt: new Date() },
      { merge: true }
    );
  } catch (error) {
